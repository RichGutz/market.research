...
class PeruMarketScraper:
    ...
    def is_valid_result(self, title: str, price: float, category: str, term: str) -> bool:
        ...
        if category == "Samsung" and "samsung" not in title_norm and "galaxy" not in title_norm:
            return False
        if category == "AirPods" and "airpods" not in title_norm:
            return False
                ...
                for item in items:
                    ...
                    try:
                        ...
                        if self.is_valid_result(title, price, category, term):
                            results.append({"store": "Mercado Libre", "title": title.strip(), "price": price, "url": link})
                    except: continue
                await browser.close()
            ...
            try:
                ...
                for item in items:
                    ...
                    try:
                        ...
                        if self.is_valid_result(title, price, category, term):
                            results.append({"store": "iShop", "title": title.strip(), "price": price, "url": link})
                    except Exception as e:
            ...
            try:
                ...
                for item in items:
                    ...
                    try:
                        ...
                        if self.is_valid_result(title, price, category, term):
                            results.append({"store": "Hiraoka", "title": title.strip(), "price": price, "url": link})
                    except: continue
                await browser.close()
            ...
            try:
                ...
                for item in items:
                    ...
                    try:
                        ...
                        if self.is_valid_result(title, price, category, term):
                            results.append({"store": "Ripley", "title": title.strip(), "price": price, "url": link})
                    except: continue
                await browser.close()
            except Exception as e:
                ...
                for item in items:
                    ...
                    try:
                        ...
                        if self.is_valid_result(title, price_clean, category, term):
                            results.append({"store": "Novox", "title": title.strip(), "price": price_clean, "url": link})
                    except Exception as e:
